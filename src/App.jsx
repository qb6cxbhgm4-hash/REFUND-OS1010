...
const NAV = [
  ...
  { id: "settings", icon: "gear", label: "Settings", sec: "Settings" },
  { id: "help", icon: "help", label: "Help Center" },
];

/* ═══ TUTORIAL ═══ */
const TUT_STEPS = [
  { target: "overview", title: "Home Dashboard", desc: "This is your command center. View real-time metrics, stock alerts, and sales data at a glance. Everything syncs live from your Shopify store.", pos: "right" },
  { target: "products", title: "Products", desc: "All your Shopify products appear here with stock levels, daily sales velocity, and days until stock-out. Search, filter, and reorder directly.", pos: "right" },
  { target: "alerts", title: "Orders & Alerts", desc: "When stock drops below your safety threshold, alerts appear here. Approve reorders with one click, or put them on hold for review.", pos: "right" },
  { target: "customers", title: "Customers", desc: "Your Shopify customer data syncs here. View order history, total spend, and customer segments to inform refund decisions.", pos: "right" },
  { target: "refunds", title: "Refunds", desc: "All refund requests from your store flow into this queue. The AI analyzes each case and recommends approve, deny, or escalate.", pos: "right" },
  { target: "inventory", title: "Inventory", desc: "Deep inventory analytics. Track stock levels across categories, see depletion forecasts, and identify which SKUs need attention.", pos: "right" },
  { target: "analytics", title: "Analytics", desc: "Sales trends, refund patterns, alert distribution, and product status breakdowns. Data-driven insights to optimize your operations.", pos: "right" },
  { target: "store", title: "Connect Your Store", desc: "This is where you connect your Shopify store. Enter your store URL, authenticate with your Admin API token, and start syncing data.", pos: "right" },
  { target: "settings", title: "Settings", desc: "Configure safety stock thresholds, lead times, minimum order quantities, and automation rules. Fine-tune how RefundOS operates for your store.", pos: "right" },
];

function Tutorial({ step, total, data, onNext, onPrev, onSkip }) {
  const el = document.querySelector(`[data-nav="${data.target}"]`);
  const rect = el ? el.getBoundingClientRect() : { top: 100, left: 60, width: 200, height: 36 };
  const cardTop = Math.min(rect.top, window.innerHeight - 280);
  const cardLeft = rect.left + rect.width + 16;
  const isMobile = window.innerWidth <= 768;
  return <>
    <div className="tut-overlay" onClick={onSkip} />
    <div className="tut-spotlight" style={{ top: rect.top - 4, left: rect.left - 4, width: rect.width + 8, height: rect.height + 8 }} />
    <div className="tut-card" style={isMobile ? { bottom: 16, left: 16, top: "auto" } : { top: cardTop, left: cardLeft }}>
      <div className="tut-progress"><div className="tut-progress-fill" style={{ width: `${((step + 1) / total) * 100}%` }} /></div>
      <div className="tut-step">Step {step + 1} of {total}</div>
      <div className="tut-title">{data.title}</div>
      <div className="tut-desc">{data.desc}</div>
      <div className="tut-actions">
        <div className="tut-dots">{Array.from({ length: total }, (_, i) => <span key={i} className={`tut-dot${i === step ? " on" : ""}`} />)}</div>
        {step > 0 && <button className="btn btn-s" onClick={onPrev}>Back</button>}
        {step < total - 1 ? <button className="btn btn-s btn-p" onClick={onNext}>Next</button> : <button className="btn btn-s btn-p" onClick={onSkip}>Get started</button>}
      </div>
    </div>
  </>;
}
...
export default function App() {
  ...
  const nav = (pg, d) => { setPage(pg); if (pg === "detail" && d) setDid(d); setSidebarOpen(false); };
  const closeSidebar = () => setSidebarOpen(false);
  const burgerIcon = sv(<><line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="18" x2="21" y2="18" /></>, 20);
  const closeIcon = sv(<><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></>, 18);
  ...
  return <TP><div style={{ display: "flex", minHeight: "100vh" }}>
    ...
    <aside className={`sidebar${sidebarOpen ? " open" : ""}`}>
      <div className="sb-head">
        <div className="sb-logo">R</div><span className="sb-brand">RefundOS</span>
        <button className="sb-close" onClick={closeSidebar}>{closeIcon}</button>
      </div>
    ...
    <div className="main">
      <div className="topbar">
        <button className="burger" onClick={() => setSidebarOpen(true)}>{burgerIcon}</button>
        <span className="tb-title">{TITLES[page] || ""}</span>
        <div className="tb-right"><Badge c="s">Connected</Badge></div>
      </div>
    ...
    {/* Tutorial */}
    {tutStep >= 0 && tutStep < TUT_STEPS.length && <Tutorial step={tutStep} total={TUT_STEPS.length} data={TUT_STEPS[tutStep]} onNext={() => setTutStep(s => s + 1)} onPrev={() => setTutStep(s => s - 1)} onSkip={() => setTutStep(-1)} />}
  </div></TP>;
}
